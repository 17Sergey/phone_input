...
class PhoneInput extends React.Component {
    ...
    componentDidMount() {
        ...
        inputField.value = this.state.mask.slice(0, this.state.maskIndex);
    }
    componentDidUpdate() {
        const inputField = document.getElementById('phone-number');
        inputField.value = this.state.mask.slice(0, this.state.maskIndex);
    ...
    phoneNumberFormatter(e) {
        ...
        */
        if (isNaN(Number(enteredSymbol)) &&
            ...
            inputField.value = mask.join("").slice(0, maskIndex);
            return;
        }
        ...
        inputField.value = currentMask.slice(0, maskIndex);
    }
    render() {
        return (
            <div className='phone_input'>
                <CounriesList flags={this.props.flags} currentFlag={this.state.flag} onCountryChange={this.handleCountryChange} />
                <input type="text" id="phone-number" onChange={this.phoneNumberFormatter} />
            </div>
        );
    }
}
